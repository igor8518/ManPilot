...
namespace PMDG_TEST
{
	...
	CRoute::CRoute()
	{
		...
		DWORD  Disposition;
		DWORD Type, Size;
		HRESULT hr = RegCreateKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\WOW6432Node\\Lockheed Martin\\Prepar3D v3", 0,NULL,0, KEY_ALL_ACCESS,NULL, &key, &Disposition);
		if (hr == ERROR_SUCCESS)
		{
			//Type = REG_SZ;
			Size = 256;
			...
			//hr = RegGetValueW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Lockheed Martin\\Prepar3D v3", L"SetupPath",0, NULL, (PVOID)path, &Size);
	...
	DSHEH *CRoute::GetSID(char* airport, char* sid)
	{
