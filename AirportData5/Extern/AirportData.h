...
struct IAirportData
{
	...
	virtual WORD GetRunwayStart(double lat, double lon, double direction) = 0;
	virtual std::vector<TPath> GetPath(DWORD start, DWORD heading, double direction) = 0;
	virtual DATA_RUNWAY GetStartCoord(WORD index) = 0;
	...
	virtual std::vector<TPath> GetPathGate(DWORD path, DWORD heading, double radius) = 0;
