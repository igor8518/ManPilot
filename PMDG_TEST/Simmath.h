#pragma once
#include "SimConnect.h"
#define _USE_MATH_DEFINES 
#include <math.h>
#include <vector>
#include <iostream>
#include <string>
#include "AirportData.h"
namespace PMDG_TEST
{
	
	enum ALTOPT
	{
		NONEALT,
		FIXALT,
		ALTORBELOW,
		ALTORABOVE,
		CRUISEALT
	};
	static class SIMMATH
	{
	public:
		static struct DSHEH
		{
			SIMCONNECT_DATA_LATLONALT Slla;
			SIMCONNECT_DATA_LATLONALT Ella;
			int fixAlt = NONEALT;
			double D;
			double SH;
			double EH;
			double H;
			double speedLimit;
			int typePath;
			std::string nameEndPoint;
			double TargetAlt = 0.0;
			double DistToAlt = 0.0;
			double Speed = 0.0;
			double Dist = 0.0;
		};
		static void DOrtoKM(DSHEH* dsheh)
		{
			double SLatitude = dsheh->Slla.Latitude / 180 * M_PI;
			double SLongitude = dsheh->Slla.Longitude / 180 * M_PI;
			double ELatitude = dsheh->Ella.Latitude / 180 * M_PI;
			double ELongitude = dsheh->Ella.Longitude / 180 * M_PI;
			double dO = (acos(sin(SLatitude)*sin(ELatitude) + cos(SLatitude)*cos(ELatitude)*cos(ELongitude - SLongitude))) * 180 / M_PI*111.12;
			if ((SLatitude == ELatitude) && (SLongitude == ELongitude))
			{
				dsheh->D = 0;
			}
			else
			{
				dsheh->D = dO;
			}
			
			dsheh->SH = M_PI_2 - atan(cos(SLatitude)*tan(ELatitude) / sin(ELongitude - SLongitude) - sin(SLatitude) / tan(ELongitude - SLongitude));
			dsheh->EH = M_PI_2 - atan(sin(ELatitude) / tan(ELongitude - SLongitude) - cos(ELatitude)*tan(SLatitude) / sin(ELongitude - SLongitude));
			dsheh->SH = dsheh->SH * 180 / M_PI;
			dsheh->EH = dsheh->EH * 180 / M_PI;
			if ((dsheh->Ella.Longitude - dsheh->Slla.Longitude) == 0)
			{
				dsheh->SH = 0;
				dsheh->EH = 0;
			}
			if (((dsheh->Ella.Longitude - dsheh->Slla.Longitude) < 0))
			{
				dsheh->SH = dsheh->SH + 180;
				dsheh->EH = dsheh->EH + 180;
			}
			else if (((dsheh->Ella.Latitude - dsheh->Slla.Latitude) == 0) && ((dsheh->Ella.Longitude - dsheh->Slla.Longitude) < 0))
			{
				dsheh->SH = dsheh->SH + 180;
				dsheh->EH = dsheh->EH + 180;
			}
			else if (((dsheh->Ella.Longitude - dsheh->Slla.Longitude) == 0) && ((dsheh->Ella.Latitude - dsheh->Slla.Latitude) < 0))
			{
				dsheh->SH = dsheh->SH + 180;
				dsheh->EH = dsheh->EH + 180;
			}
			if (dsheh->SH > 360)
			{
				dsheh->SH = dsheh->SH - 360;
			}
			if (dsheh->SH < -360)
			{
				dsheh->SH = dsheh->SH + 360;
			}
			if (dsheh->EH > 360)
			{
				dsheh->EH = dsheh->EH - 360;
			}
			if (dsheh->EH < -360)
			{
				dsheh->EH = dsheh->EH + 360;
			}

		}
		static double DecodeLat(DWORD lat)
		{

			return  90.0 - lat * (180.0 / (2 * 0x10000000));
		}

		static double DecodeLon(DWORD lon)
		{

			return lon * (360.0 / (3 * 0x10000000)) - 180.0;
		}

		static double GetAngle360(double angle)
		{
			double Tangle;
			Tangle = angle;
			while (Tangle < 0)
			{
				Tangle = Tangle + 360;
			}
			while (Tangle >= 360)
			{
				Tangle = Tangle - 360;
			}
			return Tangle;
		}

		static SIMCONNECT_DATA_LATLONALT GetDALatLon(double lat, double lon, double ang, double dist)
		{
			//$lat - текущая широта, $lng - текущая долгота
			//$dist - расстояние между искомыми точками
			//$ang - азимут на искомую точку


			dist = dist / 111.12;
			SIMCONNECT_DATA_LATLONALT latlon;
			double newlat = sin(lat * M_PI / 180)*cos(dist * M_PI / 180) + cos(lat * M_PI / 180)*sin(dist * M_PI / 180)*cos(ang * M_PI / 180);
			//newlat = lat + (asin(newlat));
			newlat = asin(newlat) * 180 / M_PI;

			double newlon = sin(dist * M_PI / 180)*sin(ang * M_PI / 180) / (cos(lat * M_PI / 180)*cos(dist * M_PI / 180) - sin(lat * M_PI / 180)*sin(dist * M_PI / 180)*cos(ang * M_PI / 180));
			newlon = lon + ((atan(newlon) * 180 / M_PI));
			latlon.Latitude = newlat;
			latlon.Longitude = newlon;
			return latlon;
		}
		/*static std::vector<TPath> solve(std::vector<TaxiwayPoints>* tpoints, std::vector<TaxiwayPaths>* tpaths, std::vector<TaxiwayNames>* tNames, std::vector<TaxiwayParks>* tParks, WORD start, WORD end, WORD run, std::string RunwayName, double heading) {
			std::string ds[] = { "", "L", "R", "C", "W", "A", "B" };
			//Максимальное число
			double INF = 10000000.0;
			std::vector<TPath> rPath;
			//Первая точка пути
			long v = start;
			//Последняя точка пути
			long t = end;
			//Количество вершин
			DWORD n = tpoints->size();
			DWORD points = tpoints->size();
			//Количество ребер
			DWORD m = tpaths->size();
			//Добавляем в матрицу паркинги
			//Добавляем в матрицу обратные пути
			for (int i = 0; i < m; i++)
			{
				if ((tpaths->at(i).Type & 0xf) == 0x3)
				{
					tpaths->at(i).IndexEndPoint = tpaths->at(i).IndexEndPoint + tpoints->size();
				}
				tpaths->push_back(tpaths->at(i));
				tpaths->at(tpaths->size() - 1).IndexEndPoint = tpaths->at(i).IndexStartPoint;
				tpaths->at(tpaths->size() - 1).IndexStartPoint = tpaths->at(i).IndexEndPoint & 0xfff;
			}
			
			for (int i = 0; i < tParks->size(); i++)
			{
				tpoints->push_back(tpoints->at(0));
				tpoints->at(tpoints->size() - 1).Lat = tParks->at(i).Lat;
				tpoints->at(tpoints->size() - 1).Lon = tParks->at(i).Lon;
			}
			//Добавляем к вершинам точку взлета
			tpoints->push_back(tpoints->at(run));

			//0 = NONE
			//1 = LEFT
			//2 = RIGHT
			//3 = CENTER
			//4 = WATER
			//5 = A
			//6 = B
			//Добавляем к ребрам участок разбега
			
				tpaths->push_back(tpaths->at(0));
				tpaths->at(tpaths->size() - 1).IndexStartPoint = end;
				tpaths->at(tpaths->size() - 1).IndexEndPoint = tpoints->size() - 1;
				tpaths->at(tpaths->size() - 1).Type = 14;
			
			//Обновляем размер матрицы ребер
			m = tpaths->size();
			//Обновляем последнюю точку
			
				end = tpoints->size() - 1;
				t = end;
			
			//Обновляем размер матрицы вершин
			n = tpoints->size();
			//Создаем матрицу весов, и заполняем ее максимальным значением
			std::vector<double> d(n, INF);
			//Вес точки старта устанавливаем в ноль
			d[v] = 0.0;
			//Создаем матрицу искомого пути
			std::vector<int> p(n, -1);
			std::vector<std::string> names(n, "");
			//Создаем матрицу углов ребер
			std::vector<double> a(n, 900.0);
			//Запускаем основной цикл согласно алгоритму
			DSHEH* dsheh = new DSHEH();

			int x;
			for (int i = 0; i<m; ++i) {
			//for (;;) {		
				//Сбрасываем флаг поиска
				bool any = false;
				//Вспомогательный цикл
				x = -1;
				for (int j = 0; j<m; ++j)
					//Если вес ребра j меньше максимального числа и тип ребра соответствует, то...
					if ((d[tpaths->at(j).IndexStartPoint] < INF) && (((tpaths->at(j).Type & 0xf) == 1)||((tpaths->at(j).Type & 0xf) == 2) || ((tpaths->at(j).Type & 0xf) == 3) || ((tpaths->at(j).Type & 0xf) == 4) ||  ((tpaths->at(j).Type & 0xf) == 14)))
					{
						//вычисляем длину ребра

						dsheh->Slla.Latitude = DecodeLat(tpoints->at(tpaths->at(j).IndexStartPoint).Lat);
						dsheh->Slla.Longitude = DecodeLon(tpoints->at(tpaths->at(j).IndexStartPoint).Lon);	
						dsheh->Ella.Latitude = DecodeLat(tpoints->at(tpaths->at(j).IndexEndPoint & 0xfff).Lat);
						dsheh->Ella.Longitude = DecodeLon(tpoints->at(tpaths->at(j).IndexEndPoint & 0xfff).Lon);

						
						DOrtoKM(dsheh);
						
						//Если тип ребра "ВПП", то...
						if ((tpaths->at(j).Type & 0xf) == 2)
						{
							//умножаем длину ребра на коэффициент
							dsheh->D = dsheh->D * 10000;
						}
						if ((tpaths->at(j).Type & 0xf) == 3)
						{
							//умножаем длину ребра на коэффициент
							dsheh->D = dsheh->D * 100000;
						}
						//Если вес ребра отрицательный, то...
						if (d[tpaths->at(j).IndexStartPoint] <= 0)
						{
							//увеличиваем вес ребра на половину от максимального значения
							dsheh->D = int(INF / 10) - dsheh->D;
						}
						double ra;
						//Если угол еще не обновлялся, то
						if (a[tpaths->at(j).IndexStartPoint] == 900.0)
						{
							ra = dsheh->EH - heading;
							if (ra > 180)
							{
								ra = ra - 360;
							}
							else if (ra < -180)
							{
								ra = ra + 360;
							}
							a[tpaths->at(j).IndexStartPoint] = ra;
						}
						else
						{
							ra = dsheh->EH - a[tpaths->at(j).IndexStartPoint];
							if (ra > 180)
							{
								ra = ra - 360;
							}
							else if (ra < -180)
							{
								ra = ra + 360;
							}
							//a[tpaths->at(j).IndexStartPoint] = ra;
						}
						double RRA;
						if (a[tpaths->at(j).IndexStartPoint] != 900.0)
						{
							RRA = abs(pow(abs(ra), 4)) / 100000.0;
						}
						else
						{
							RRA = RRA = abs(pow(abs(ra), 4)) / 100000.0;
						}
						//Если прошлая найденная сумма весов ребер больше текущей, то...
						if (d[tpaths->at(j).IndexEndPoint & 0xfff] > (d[tpaths->at(j).IndexStartPoint] + dsheh->D + RRA)) {



								//d[tpaths->at(j).IndexEndPoint & 0xfff] = d[tpaths->at(j).IndexStartPoint] + dsheh->D + abs(pow(abs(ra),4))/100000.0;
							d[tpaths->at(j).IndexEndPoint & 0xfff] = max(-INF, d[tpaths->at(j).IndexStartPoint] + dsheh->D + RRA);

							a[tpaths->at(j).IndexEndPoint & 0xfff] = dsheh->EH;
							/*bool ff = false;
							for (int cur = (tpaths->at(j).IndexStartPoint); cur != -1; cur = p[cur])
							{
								if (p[cur] == (tpaths->at(j).IndexEndPoint & 0xfff))
								{
									p.push_back(tpaths->at(j).IndexStartPoint);
									ff = true;
								}

							}
							if (!ff)
							{
								p[tpaths->at(j).IndexEndPoint & 0xfff] = tpaths->at(j).IndexStartPoint;
							}*/
							/*p[tpaths->at(j).IndexEndPoint & 0xfff] = tpaths->at(j).IndexStartPoint;
							x = tpaths->at(j).IndexEndPoint & 0xfff;

							if (tpaths->at(j).Type == 2)
							{
								std::string R1 = "";
								if ((tpaths->at(j).TaxiNameIndex) < 10)
								{
									R1 = "0" + std::to_string(tpaths->at(j).TaxiNameIndex) + ds[tpaths->at(j).IndexEndPoint >> 12];
								}
								else
								{
									R1 = std::to_string(tpaths->at(j).TaxiNameIndex) + ds[tpaths->at(j).IndexEndPoint >> 12];
								}
								names[tpaths->at(j).IndexEndPoint & 0xfff] = R1;
							}
							else if (tpaths->at(j).Type == 14)
							{
								names[tpaths->at(j).IndexEndPoint & 0xfff] = RunwayName;
							}
							else 
							{							
								names[tpaths->at(j).IndexEndPoint & 0xfff] = tNames->at(tpaths->at(j).TaxiNameIndex).TaxiName;
							}
							any = true;
						}
						
					}
				
				if (!any)  break;
			}
			delete dsheh;
			dsheh = NULL;
			int z = 0;
			if (x == -1)
				std::cout << "No negative cycle from " << v;
			else {
				int y = x;
				for (int i = 0; i < n; ++i)
				{
					if (y >= 0)
					{
						z = y;
						y = p[y];
					}
					else
					{
						std::cout << "!!! ";
					}
				}

				std::vector<int> path;
				for (int cur = y; ; cur = p[cur]) {
					path.push_back(cur);
					if (cur == y && path.size() > 1)  break;
				}
				reverse(path.begin(), path.end());

				std::cout << "Negative cycle: ";
				for (size_t i = 0; i<path.size(); ++i)
					std::cout << path[i] << ' ';
			}
			if (d[t] >= INF)
			{
				std::cout << std::endl << v << "->" << t << "=" << "Not";
			}
			else
			{
				std::cout << std::endl << v << "->" << t << "=" << d[t];
			}
			if (d[t] == INF)
				std::cout << std::endl << "No path from " << v << " to " << t << ".";
			else {
				std::vector<int> path;
				for (int cur = t; cur != -1; cur = p[cur])
				{
					path.push_back(cur);
					
				}
				reverse(path.begin(), path.end());

				std::cout << std::endl << "Path from " << v << " to " << t << ": ";
				for (int i = 0; i < path.size(); ++i)
				{
					std::cout << path[i] << ' ';
					TPath sPath;
					sPath.index = i;
					sPath.name = names.at(path[i]);
					sPath.Lat = DecodeLat(tpoints->at(path[i]).Lat);
					sPath.Lon = DecodeLon(tpoints->at(path[i]).Lon);
					sPath.Type = 0;
					rPath.push_back(sPath);
				}
				PMDG_TEST::SIMMATH::DSHEH dsheh = {
					DecodeLat(tpoints->at(path[0]).Lat),
					DecodeLon(tpoints->at(path[0]).Lon), 0.0,
					DecodeLat(tpoints->at(path[1]).Lat),
					DecodeLon(tpoints->at(path[1]).Lon), 0.0
				};
				PMDG_TEST::SIMMATH::DOrtoKM(&dsheh);
				double RAngle = dsheh.EH - heading;
				if (RAngle > 180)
				{
					RAngle = RAngle - 360;
				}
				else if (RAngle < -180)
				{
					RAngle = RAngle + 360;
				}
				if (abs(RAngle > 90.0))
				{
					for (int i = 0; i < path.size(); i++)
					{

					}
				}
				if (path.size() > 0)
				{
					PMDG_TEST::SIMMATH::DSHEH dsheh = {
						DecodeLat(tpoints->at(path[0]).Lat),
						DecodeLon(tpoints->at(path[0]).Lon), 0.0,
						DecodeLat(tpoints->at(path[1]).Lat),
						DecodeLon(tpoints->at(path[1]).Lon), 0.0
					};
					PMDG_TEST::SIMMATH::DOrtoKM(&dsheh);
					double RAngle = dsheh.EH - heading;
					if (RAngle > 180)
					{
						RAngle = RAngle - 360;
					}
					else if (RAngle < -180)
					{
						RAngle = RAngle + 360;
					}
					if (abs(RAngle) > (90.0))
					{
						int i = 1;
						rPath[0].Type = 11;
						while (true)
						{
							PMDG_TEST::SIMMATH::DSHEH dshehS = {
								DecodeLat(tpoints->at(path[i - 1]).Lat),
								DecodeLon(tpoints->at(path[i - 1]).Lon), 0.0,
								DecodeLat(tpoints->at(path[i]).Lat),
								DecodeLon(tpoints->at(path[i]).Lon), 0.0
							};
							PMDG_TEST::SIMMATH::DOrtoKM(&dshehS);
							double RA = 360.0;
							int index = tpaths->size();
							for (int j = 0; j < tpaths->size(); j++)
							{
								if ((tpaths->at(j).IndexStartPoint == path[i])&&
									((tpaths->at(j).Type & 0xf) != 0x3) &&
									((tpaths->at(j).IndexEndPoint & 0xfff) != path[i-1]) &&
									((tpaths->at(j).IndexEndPoint & 0xfff) != path[i + 1]))
								{
									PMDG_TEST::SIMMATH::DSHEH dshehE = {
										DecodeLat(tpoints->at(tpaths->at(j).IndexStartPoint).Lat),
										DecodeLon(tpoints->at(tpaths->at(j).IndexStartPoint).Lon), 0.0,
										DecodeLat(tpoints->at((tpaths->at(j).IndexEndPoint & 0xfff)).Lat),
										DecodeLon(tpoints->at((tpaths->at(j).IndexEndPoint & 0xfff)).Lon), 0.0
									};
									PMDG_TEST::SIMMATH::DOrtoKM(&dshehE);
									double RAngle = dshehS.EH - dshehE.EH;
									if (RAngle > 180)
									{
										RAngle = RAngle - 360;
									}
									else if (RAngle < -180)
									{
										RAngle = RAngle + 360;
									}
									if (abs(RAngle) < (105.0))
									{
										if (abs(RAngle) < RA)
										{
											RA = abs(RAngle);
											index = j;
										}
									}
								}
							}
							PMDG_TEST::SIMMATH::DSHEH dsheh;
							if (index != tpaths->size())
							{
								/*dsheh = {
									DecodeLat(tpoints->at(tpaths->at(index).IndexStartPoint).Lat),
									DecodeLon(tpoints->at(tpaths->at(index).IndexStartPoint).Lon), 0.0,
									DecodeLat(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lat),
									DecodeLon(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lon), 0.0
								};
								
								PMDG_TEST::SIMMATH::DOrtoKM(&dsheh);*/
								/*rPath[i].Type = 11;
								TPath sPath;
								sPath.index = i;
								sPath.name = names.at(path[i]);
								sPath.Lat = DecodeLat(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lat);
								sPath.Lon = DecodeLon(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lon);
								sPath.Type = 11;
								rPath.insert(rPath.begin() + i + 1, sPath);
								break;
							}
							else
							{
								rPath[i].Type = 11;
								i++;
							}
							
							//path[i]
							

						}
					}
				}
				
			}
			
			return rPath;
		}*/
		//static std::vector<double>* d;
		static std::vector<TPath> solve(std::vector<TaxiwayPoints>* tpoints, std::vector<TaxiwayPaths>* tpaths, std::vector<TaxiwayNames>* tNames, std::vector<TaxiwayParks>* tParks, WORD start, WORD end, WORD run, std::string RunwayName, double heading, INT64* dR) {
			std::string ds[] = { "", "L", "R", "C", "W", "A", "B" };
			//Максимальное число
			double INF = 100000000.0;
			std::vector<TPath> rPath;
			//Первая точка пути
			long v = start;
			//Последняя точка пути
			long t = end;
			//Количество вершин
			DWORD n = tpoints->size();
			DWORD points = tpoints->size();
			//Количество ребер
			DWORD m = tpaths->size();
			//Добавляем в матрицу паркинги
			//Добавляем в матрицу обратные пути
			for (int i = 0; i < m; i++)
			{
				if ((tpaths->at(i).Type & 0xf) == 0x3)
				{
					tpaths->at(i).IndexEndPoint = tpaths->at(i).IndexEndPoint + tpoints->size();
				}
				tpaths->push_back(tpaths->at(i));
				tpaths->at(tpaths->size() - 1).IndexEndPoint = tpaths->at(i).IndexStartPoint;
				tpaths->at(tpaths->size() - 1).IndexStartPoint = tpaths->at(i).IndexEndPoint & 0xfff;
			}

			for (int i = 0; i < tParks->size(); i++)
			{
				tpoints->push_back(tpoints->at(0));
				tpoints->at(tpoints->size() - 1).Lat = tParks->at(i).Lat;
				tpoints->at(tpoints->size() - 1).Lon = tParks->at(i).Lon;
			}
			//Добавляем к вершинам точку взлета
			tpoints->push_back(tpoints->at(run));

			//0 = NONE
			//1 = LEFT
			//2 = RIGHT
			//3 = CENTER
			//4 = WATER
			//5 = A
			//6 = B
			//Добавляем к ребрам участок разбега

			tpaths->push_back(tpaths->at(0));
			tpaths->at(tpaths->size() - 1).IndexStartPoint = end;
			tpaths->at(tpaths->size() - 1).IndexEndPoint = tpoints->size() - 1;
			tpaths->at(tpaths->size() - 1).Type = 14;

			//Обновляем размер матрицы ребер
			m = tpaths->size();
			//Обновляем последнюю точку

			end = tpoints->size() - 1;
			t = end;

			//Обновляем размер матрицы вершин
			n = tpoints->size();
			//Создаем матрицу весов, и заполняем ее максимальным значением
			std::vector<double>* d = new std::vector<double>(n, INF);
			//Вес точки старта устанавливаем в ноль
			d->at(v) = 0.0;
			//Создаем матрицу искомого пути
			std::vector<int> p(n, -1);
			std::vector<std::string> names(n, "");
			//Создаем матрицу углов ребер
			std::vector<double> a(n, 180.0);
			a[v] = heading;
			//Запускаем основной цикл согласно алгоритму
			DSHEH* dsheh = new DSHEH();

			int x;
			//for (int i = 0; i<m; ++i) {
				for (;;) {		
				//Сбрасываем флаг поиска
				bool any = false;
				//Вспомогательный цикл
				x = -1;
				for (int j = 0; j < m; ++j)
				{
					if (j == 511)
					{
						std::cout << 0;
					}
					//Если вес ребра j меньше максимального числа и тип ребра соответствует, то...
					if ((d->at(tpaths->at(j).IndexStartPoint) < INF) && (((tpaths->at(j).Type & 0xf) == 1) || ((tpaths->at(j).Type & 0xf) == 2) || ((tpaths->at(j).Type & 0xf) == 3) || ((tpaths->at(j).Type & 0xf) == 4) || ((tpaths->at(j).Type & 0xf) == 14)))
					{
						//вычисляем длину ребра

						dsheh->Slla.Latitude = DecodeLat(tpoints->at(tpaths->at(j).IndexStartPoint).Lat);
						dsheh->Slla.Longitude = DecodeLon(tpoints->at(tpaths->at(j).IndexStartPoint).Lon);
						dsheh->Ella.Latitude = DecodeLat(tpoints->at(tpaths->at(j).IndexEndPoint & 0xfff).Lat);
						dsheh->Ella.Longitude = DecodeLon(tpoints->at(tpaths->at(j).IndexEndPoint & 0xfff).Lon);


						DOrtoKM(dsheh);

						//Если тип ребра "ВПП", то...
						if ((tpaths->at(j).Type & 0xf) == 2)
						{
							//умножаем длину ребра на коэффициент
							dsheh->D = dsheh->D * 1000000;
						}
						if ((tpaths->at(j).Type & 0xf) == 3)
						{
							//умножаем длину ребра на коэффициент
							dsheh->D = dsheh->D * 100000;
						}
						//Если вес ребра отрицательный, то...
						if (d->at(tpaths->at(j).IndexStartPoint) < 0)
						{
							//увеличиваем вес ребра на половину от максимального значения
							dsheh->D = int(INF / 10) - dsheh->D;
						}
						double ra;
						//if (a[tpaths->at(j).IndexStartPoint] == 900.0)
						{
							//ra = a[tpaths->at(j).IndexStartPoint];
						}
						int CountRa = 1;
						//else
						{
							ra = dsheh->EH - a[tpaths->at(j).IndexStartPoint];
							if (ra > 180)
							{
								ra = ra - 360;
							}
							else if (ra < -180)
							{
								ra = ra + 360;
							}
							ra = abs(ra);
							double DComm = 0.050;
							DWORD lastPoint = tpaths->at(j).IndexStartPoint;
							
							for (int cur = tpaths->at(j).IndexStartPoint; cur != -1; cur = p[cur])
							{
								PMDG_TEST::SIMMATH::DSHEH de = {
								de.Slla.Latitude = DecodeLat(tpoints->at(lastPoint).Lat),
								de.Slla.Longitude = DecodeLon(tpoints->at(lastPoint).Lon), 0.0,
								de.Ella.Latitude = DecodeLat(tpoints->at(cur).Lat),
								de.Ella.Longitude = DecodeLon(tpoints->at(cur).Lon), 0.0 };
								DOrtoKM(&de);
								DComm = DComm - de.D;
								if (DComm > 0.0)
								{
									if (de.D != 0.0)
									{
										double ra2 = a[lastPoint] - a[cur];
										if (ra2 > 180)
										{
											ra2 = ra2 - 360;
										}
										else if (ra2 < -180)
										{
											ra2 = ra2 + 360;
										}
										CountRa = CountRa + 1;
										ra = ra + abs(ra2);

									}
									lastPoint = cur;
								}
								else
								{
									break;
								}

							}
						}
						ra = ra / CountRa;
						double RRA;

						RRA = abs(pow(abs(ra), 3)) / 100.0;

						//Если прошлая найденная сумма весов ребер больше текущей, то...
						if (d->at(tpaths->at(j).IndexEndPoint & 0xfff) > (d->at(tpaths->at(j).IndexStartPoint) + dsheh->D + RRA)) {

							//d[tpaths->at(j).IndexEndPoint & 0xfff] = d[tpaths->at(j).IndexStartPoint] + dsheh->D + abs(pow(abs(ra),4))/100000.0;
							if ((tpaths->at(j).IndexEndPoint & 0xfff) == 511)
							{
								std::cout << 0;
							}
							d->at(tpaths->at(j).IndexEndPoint & 0xfff) = max(-INF, d->at(tpaths->at(j).IndexStartPoint) + dsheh->D + RRA);

							a[tpaths->at(j).IndexEndPoint & 0xfff] = dsheh->EH;

							p[tpaths->at(j).IndexEndPoint & 0xfff] = tpaths->at(j).IndexStartPoint;
							x = tpaths->at(j).IndexEndPoint & 0xfff;

							if ((tpaths->at(j).Type & 0xf) == 2)
							{
								std::string R1 = "";
								if ((tpaths->at(j).TaxiNameIndex) < 10)
								{
									R1 = "0" + std::to_string(tpaths->at(j).TaxiNameIndex) + ds[tpaths->at(j).IndexEndPoint >> 12];
								}
								else
								{
									R1 = std::to_string(tpaths->at(j).TaxiNameIndex) + ds[tpaths->at(j).IndexEndPoint >> 12];
								}
								names[tpaths->at(j).IndexEndPoint & 0xfff] = R1;
							}
							else if ((tpaths->at(j).Type & 0xf) == 14)
							{
								names[tpaths->at(j).IndexEndPoint & 0xfff] = RunwayName;
							}
							else
							{
								names[tpaths->at(j).IndexEndPoint & 0xfff] = tNames->at(tpaths->at(j).TaxiNameIndex).TaxiName;
							}
							any = true;
						}

					}
				}
				if (!any)
				{
					break;
				}
			}
			delete dsheh;
			dsheh = NULL;
			int z = 0;
			if (x == -1)
				std::cout << "No negative cycle from " << v;
			else {
				int y = x;
				for (int i = 0; i < n; ++i)
				{
					if (y >= 0)
					{
						z = y;
						y = p[y];
					}
					else
					{
						std::cout << "!!! ";
					}
				}

				std::vector<int> path;
				for (int cur = y; ; cur = p[cur]) {
					path.push_back(cur);
					if (cur == y && path.size() > 1)  break;
				}
				reverse(path.begin(), path.end());

				std::cout << "Negative cycle: ";
				for (size_t i = 0; i<path.size(); ++i)
					std::cout << path[i] << ' ';
			}
			if (d->at(t) >= INF)
			{
				std::cout << std::endl << v << "->" << t << "=" << "Not";
			}
			else
			{
				std::cout << std::endl << v << "->" << t << "=" << d->at(t);
			}
			if (d->at(t) == INF)
				std::cout << std::endl << "No path from " << v << " to " << t << ".";
			else {
				std::vector<int> path;
				for (int cur = t; cur != -1; cur = p[cur])
				{
					path.push_back(cur);

				}
				reverse(path.begin(), path.end());

				std::cout << std::endl << "Path from " << v << " to " << t << ": ";
				for (int i = 0; i < path.size(); ++i)
				{
					std::cout << path[i] << ' ';
					TPath sPath;
					sPath.index = i;
					sPath.name = names.at(path[i]);
					sPath.Lat = DecodeLat(tpoints->at(path[i]).Lat);
					sPath.Lon = DecodeLon(tpoints->at(path[i]).Lon);
					sPath.Type = 0;
					rPath.push_back(sPath);
				}
				PMDG_TEST::SIMMATH::DSHEH dsheh = {
					DecodeLat(tpoints->at(path[0]).Lat),
					DecodeLon(tpoints->at(path[0]).Lon), 0.0,
					DecodeLat(tpoints->at(path[1]).Lat),
					DecodeLon(tpoints->at(path[1]).Lon), 0.0
				};
				PMDG_TEST::SIMMATH::DOrtoKM(&dsheh);
				double RAngle = dsheh.EH - heading;
				if (RAngle > 180)
				{
					RAngle = RAngle - 360;
				}
				else if (RAngle < -180)
				{
					RAngle = RAngle + 360;
				}
				if (abs(RAngle > 90.0))
				{
					for (int i = 0; i < path.size(); i++)
					{

					}
				}
				if (path.size() > 0)
				{
					PMDG_TEST::SIMMATH::DSHEH dsheh = {
						DecodeLat(tpoints->at(path[0]).Lat),
						DecodeLon(tpoints->at(path[0]).Lon), 0.0,
						DecodeLat(tpoints->at(path[1]).Lat),
						DecodeLon(tpoints->at(path[1]).Lon), 0.0
					};
					PMDG_TEST::SIMMATH::DOrtoKM(&dsheh);
					double RAngle = dsheh.EH - heading;
					if (RAngle > 180)
					{
						RAngle = RAngle - 360;
					}
					else if (RAngle < -180)
					{
						RAngle = RAngle + 360;
					}
					if (abs(RAngle) >(90.0))
					{
						int i = 1;
						rPath[0].Type = 11;
						while (true)
						{
							PMDG_TEST::SIMMATH::DSHEH dshehS = {
								DecodeLat(tpoints->at(path[i - 1]).Lat),
								DecodeLon(tpoints->at(path[i - 1]).Lon), 0.0,
								DecodeLat(tpoints->at(path[i]).Lat),
								DecodeLon(tpoints->at(path[i]).Lon), 0.0
							};
							PMDG_TEST::SIMMATH::DOrtoKM(&dshehS);
							double RA = 360.0;
							int index = tpaths->size();
							for (int j = 0; j < tpaths->size(); j++)
							{
								if ((tpaths->at(j).IndexStartPoint == path[i]) &&
									((tpaths->at(j).Type & 0xf) != 0x3) &&
									((tpaths->at(j).IndexEndPoint & 0xfff) != path[i - 1]) &&
									((tpaths->at(j).IndexEndPoint & 0xfff) != path[i + 1]))
								{
									PMDG_TEST::SIMMATH::DSHEH dshehE = {
										DecodeLat(tpoints->at(tpaths->at(j).IndexStartPoint).Lat),
										DecodeLon(tpoints->at(tpaths->at(j).IndexStartPoint).Lon), 0.0,
										DecodeLat(tpoints->at((tpaths->at(j).IndexEndPoint & 0xfff)).Lat),
										DecodeLon(tpoints->at((tpaths->at(j).IndexEndPoint & 0xfff)).Lon), 0.0
									};
									PMDG_TEST::SIMMATH::DOrtoKM(&dshehE);
									double RAngle = dshehS.EH - dshehE.EH;
									if (RAngle > 180)
									{
										RAngle = RAngle - 360;
									}
									else if (RAngle < -180)
									{
										RAngle = RAngle + 360;
									}
									if (abs(RAngle) < (105.0))
									{
										if (abs(RAngle) < RA)
										{
											RA = abs(RAngle);
											index = j;
										}
									}
								}
							}
							PMDG_TEST::SIMMATH::DSHEH dsheh;
							if (index != tpaths->size())
							{
								/*dsheh = {
								DecodeLat(tpoints->at(tpaths->at(index).IndexStartPoint).Lat),
								DecodeLon(tpoints->at(tpaths->at(index).IndexStartPoint).Lon), 0.0,
								DecodeLat(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lat),
								DecodeLon(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lon), 0.0
								};

								PMDG_TEST::SIMMATH::DOrtoKM(&dsheh);*/
								rPath[i].Type = 11;
								TPath sPath;
								sPath.index = path[i];
								sPath.name = names.at(path[i]);
								sPath.Lat = DecodeLat(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lat);
								sPath.Lon = DecodeLon(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lon);
								sPath.Type = 11;
								rPath.insert(rPath.begin() + i + 1, sPath);
								break;
							}
							else
							{
								rPath[i].Type = 11;
								i++;
							}

							//path[i]


						}
					}
				}

			}
			*dR = (INT64)d;
			return rPath;
		}

		static std::vector<TPath> solveGate(std::vector<TaxiwayPoints>* tpoints, std::vector<TaxiwayPaths>* tpaths, std::vector<TaxiwayNames>* tNames, std::vector<TaxiwayParks>* tParks, WORD start, WORD end, double heading) {
			std::string ds[] = { "", "L", "R", "C", "W", "A", "B" };
			//Максимальное число
			double INF = 100000000.0;
			std::vector<TPath> rPath;
			//Первая точка пути
			long v = start;
			//Последняя точка пути
			long t = end + tpoints->size() - tParks->size();



			//Обновляем размер матрицы ребер
			DWORD m = tpaths->size();
	

			//Обновляем размер матрицы вершин
			DWORD n = tpoints->size();
			//Создаем матрицу весов, и заполняем ее максимальным значением
			std::vector<double> d(n, INF);
			//Вес точки старта устанавливаем в ноль
			d[v] = 0.0;
			
			//Создаем матрицу искомого пути
			std::vector<int> p(n, -1);
			std::vector<std::string> names(n, "");
			//Создаем матрицу углов ребер
			std::vector<double> a(n, 180.0);
			a[v] = heading;
			//Запускаем основной цикл согласно алгоритму
			DSHEH* dsheh = new DSHEH();

			int x;
			for (int i = 0; i<m; ++i) {
				//for (;;) {		
				//Сбрасываем флаг поиска
				bool any = false;
				//Вспомогательный цикл
				x = -1;
				for (int j = 0; j<m; ++j)
					//Если вес ребра j меньше максимального числа и тип ребра соответствует, то...
					if ((d[tpaths->at(j).IndexStartPoint] < INF) && (((tpaths->at(j).Type & 0xf) == 1) || ((tpaths->at(j).Type & 0xf) == 2) || ((tpaths->at(j).Type & 0xf) == 3) || ((tpaths->at(j).Type & 0xf) == 4) || ((tpaths->at(j).Type & 0xf) == 14)))
					{
						//вычисляем длину ребра

						dsheh->Slla.Latitude = DecodeLat(tpoints->at(tpaths->at(j).IndexStartPoint).Lat);
						dsheh->Slla.Longitude = DecodeLon(tpoints->at(tpaths->at(j).IndexStartPoint).Lon);
						dsheh->Ella.Latitude = DecodeLat(tpoints->at(tpaths->at(j).IndexEndPoint & 0xfff).Lat);
						dsheh->Ella.Longitude = DecodeLon(tpoints->at(tpaths->at(j).IndexEndPoint & 0xfff).Lon);

						if (tpaths->at(j).IndexStartPoint == 22)
						{
							std::cout << 1;
						}
						DOrtoKM(dsheh);

						//Если тип ребра "ВПП", то...
						if ((tpaths->at(j).Type & 0xf) == 2)
						{
							//умножаем длину ребра на коэффициент
							dsheh->D = dsheh->D * 1000000;
						}
						if ((tpaths->at(j).Type & 0xf) == 3)
						{
							//умножаем длину ребра на коэффициент
							dsheh->D = dsheh->D * 100000;
						}
						//Если вес ребра отрицательный, то...
						if (d[tpaths->at(j).IndexStartPoint] < 0)
						{
							//увеличиваем вес ребра на половину от максимального значения
							dsheh->D = int(INF / 10) - dsheh->D;
						}
						double ra;
						//if (a[tpaths->at(j).IndexStartPoint] == 900.0)
						{
							//ra = a[tpaths->at(j).IndexStartPoint];
						}
						//else
						int CountRa = 1;
						{
							ra = dsheh->EH - a[tpaths->at(j).IndexStartPoint];
							if (ra > 180)
							{
								ra = ra - 360;
							}
							else if (ra < -180)
							{
								ra = ra + 360;
							}
							ra = abs(ra);
							double DComm = 0.050;
							DWORD lastPoint = tpaths->at(j).IndexStartPoint;
							for (int cur = tpaths->at(j).IndexStartPoint; cur != -1; cur = p[cur])
							{
								PMDG_TEST::SIMMATH::DSHEH de;
								de.Slla.Latitude = DecodeLat(tpoints->at(lastPoint).Lat);
								de.Slla.Longitude = DecodeLon(tpoints->at(lastPoint).Lon);
								de.Ella.Latitude = DecodeLat(tpoints->at(cur).Lat);
								de.Ella.Longitude = DecodeLon(tpoints->at(cur).Lon);
								DOrtoKM(&de);
								DComm = DComm - de.D;
								if (DComm > 0.0)
								{
									if (de.D != 0.0)
									{
										double ra2 = a[lastPoint] - a[cur];
										if (ra2 > 180)
										{
											ra2 = ra2 - 360;
										}
										else if (ra2 < -180)
										{
											ra2 = ra2 + 360;
										}
										CountRa = CountRa + 1;
										ra = ra + abs(ra2);

									}
									lastPoint = cur;
								}
								else
								{
									break;
								}

							}
						}
						ra = ra / CountRa;
						double RRA;

						RRA = abs(pow(abs(ra), 5)) / 100000.0;
	
						//Если прошлая найденная сумма весов ребер больше текущей, то...
						if (d[tpaths->at(j).IndexEndPoint & 0xfff] > (d[tpaths->at(j).IndexStartPoint] + dsheh->D + RRA)) {

							//d[tpaths->at(j).IndexEndPoint & 0xfff] = d[tpaths->at(j).IndexStartPoint] + dsheh->D + abs(pow(abs(ra),4))/100000.0;
							d[tpaths->at(j).IndexEndPoint & 0xfff] = max(-INF, d[tpaths->at(j).IndexStartPoint] + dsheh->D + RRA);

							a[tpaths->at(j).IndexEndPoint & 0xfff] = dsheh->EH;

							p[tpaths->at(j).IndexEndPoint & 0xfff] = tpaths->at(j).IndexStartPoint;
							x = tpaths->at(j).IndexEndPoint & 0xfff;

							if ((tpaths->at(j).Type & 0xf) == 2)
							{
								std::string R1 = "";
								if ((tpaths->at(j).TaxiNameIndex) < 10)
								{
									R1 = "0" + std::to_string(tpaths->at(j).TaxiNameIndex) + ds[tpaths->at(j).IndexEndPoint >> 12];
								}
								else
								{
									R1 = std::to_string(tpaths->at(j).TaxiNameIndex) + ds[tpaths->at(j).IndexEndPoint >> 12];
								}
								names[tpaths->at(j).IndexEndPoint & 0xfff] = R1;
							}
							else
							{
								names[tpaths->at(j).IndexEndPoint & 0xfff] = tNames->at(tpaths->at(j).TaxiNameIndex).TaxiName;
							}
							any = true;
						}

					}

				if (!any)  break;
			}
			delete dsheh;
			dsheh = NULL;
			int z = 0;
			if (x == -1)
				std::cout << "No negative cycle from " << v;
			else {
				int y = x;
				for (int i = 0; i < n; ++i)
				{
					if (y >= 0)
					{
						z = y;
						y = p[y];
					}
					else
					{
						std::cout << "!!! ";
					}
				}

				std::vector<int> path;
				for (int cur = y; ; cur = p[cur]) {
					path.push_back(cur);
					if (cur == y && path.size() > 1)  break;
				}
				reverse(path.begin(), path.end());

				std::cout << "Negative cycle: ";
				for (size_t i = 0; i<path.size(); ++i)
					std::cout << path[i] << ' ';
			}
			if (d[t] >= INF)
			{
				std::cout << std::endl << v << "->" << t << "=" << "Not";
			}
			else
			{
				std::cout << std::endl << v << "->" << t << "=" << d[t];
			}
			if (d[t] == INF)
				std::cout << std::endl << "No path from " << v << " to " << t << ".";
			else {
				std::vector<int> path;
				for (int cur = t; cur != -1; cur = p[cur])
				{
					path.push_back(cur);

				}
				reverse(path.begin(), path.end());

				std::cout << std::endl << "Path from " << v << " to " << t << ": ";
				for (int i = 0; i < path.size(); ++i)
				{
					std::cout << path[i] << ' ';
					TPath sPath;
					sPath.index = sPath.index = path[i];;
					sPath.name = names.at(path[i]);
					sPath.Lat = DecodeLat(tpoints->at(path[i]).Lat);
					sPath.Lon = DecodeLon(tpoints->at(path[i]).Lon);
					sPath.Type = 0;
					rPath.push_back(sPath);
				}
				/*PMDG_TEST::SIMMATH::DSHEH dsheh = {
					DecodeLat(tpoints->at(path[0]).Lat),
					DecodeLon(tpoints->at(path[0]).Lon), 0.0,
					DecodeLat(tpoints->at(path[1]).Lat),
					DecodeLon(tpoints->at(path[1]).Lon), 0.0
				};
				PMDG_TEST::SIMMATH::DOrtoKM(&dsheh);
				double RAngle = dsheh.EH - heading;
				if (RAngle > 180)
				{
					RAngle = RAngle - 360;
				}
				else if (RAngle < -180)
				{
					RAngle = RAngle + 360;
				}
				if (abs(RAngle > 90))
				{
					for (int i = 0; i < path.size(); i++)
					{

					}
				}
				if (path.size() > 0)
				{
					PMDG_TEST::SIMMATH::DSHEH dsheh = {
						DecodeLat(tpoints->at(path[0]).Lat),
						DecodeLon(tpoints->at(path[0]).Lon), 0.0,
						DecodeLat(tpoints->at(path[1]).Lat),
						DecodeLon(tpoints->at(path[1]).Lon), 0.0
					};
					PMDG_TEST::SIMMATH::DOrtoKM(&dsheh);
					double RAngle = dsheh.EH - heading;;
					if (RAngle > 180)
					{
						RAngle = RAngle - 360;
					}
					else if (RAngle < -180)
					{
						RAngle = RAngle + 360;
					}
					if (abs(RAngle) >(90.0))
					{
						int i = 1;
						rPath[0].Type = 11;
						while (true)
						{
							PMDG_TEST::SIMMATH::DSHEH dshehS = {
								DecodeLat(tpoints->at(path[i - 1]).Lat),
								DecodeLon(tpoints->at(path[i - 1]).Lon), 0.0,
								DecodeLat(tpoints->at(path[i]).Lat),
								DecodeLon(tpoints->at(path[i]).Lon), 0.0
							};
							PMDG_TEST::SIMMATH::DOrtoKM(&dshehS);
							double RA = 360.0;
							int index = tpaths->size();
							for (int j = 0; j < tpaths->size(); j++)
							{
								if ((tpaths->at(j).IndexStartPoint == path[i]) &&
									((tpaths->at(j).Type & 0xf) != 0x3) &&
									((tpaths->at(j).IndexEndPoint & 0xfff) != path[i - 1]) &&
									((tpaths->at(j).IndexEndPoint & 0xfff) != path[i + 1]))
								{
									PMDG_TEST::SIMMATH::DSHEH dshehE = {
										DecodeLat(tpoints->at(tpaths->at(j).IndexStartPoint).Lat),
										DecodeLon(tpoints->at(tpaths->at(j).IndexStartPoint).Lon), 0.0,
										DecodeLat(tpoints->at((tpaths->at(j).IndexEndPoint & 0xfff)).Lat),
										DecodeLon(tpoints->at((tpaths->at(j).IndexEndPoint & 0xfff)).Lon), 0.0
									};
									PMDG_TEST::SIMMATH::DOrtoKM(&dshehE);
									double RAngle = dshehS.EH - dshehE.EH;
									if (RAngle > 180)
									{
										RAngle = RAngle - 360;
									}
									else if (RAngle < -180)
									{
										RAngle = RAngle + 360;
									}
									if (abs(RAngle) < (100.0))
									{
										if (abs(RAngle) < RA)
										{
											RA = abs(RAngle);
											index = j;
										}
									}
								}
							}
							PMDG_TEST::SIMMATH::DSHEH dsheh;
							if (index != tpaths->size())
							{
								/*dsheh = {
								DecodeLat(tpoints->at(tpaths->at(index).IndexStartPoint).Lat),
								DecodeLon(tpoints->at(tpaths->at(index).IndexStartPoint).Lon), 0.0,
								DecodeLat(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lat),
								DecodeLon(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lon), 0.0
								};

								PMDG_TEST::SIMMATH::DOrtoKM(&dsheh);
								rPath[i].Type = 11;
								TPath sPath;
								sPath.index = i;
								sPath.name = names.at(path[i]);
								sPath.Lat = DecodeLat(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lat);
								sPath.Lon = DecodeLon(tpoints->at((tpaths->at(index).IndexEndPoint & 0xfff)).Lon);
								sPath.Type = 11;
								rPath.insert(rPath.begin() + i + 1, sPath);
								break;
							}
							else
							{
								rPath[i].Type = 11;
								i++;
							}

							//path[i]


						}
					}
				}*/

			}

			return rPath;
		}
		
	};
}
