...
#define PMDG_ADD_DATA_DEFINITION	0x4E877778
	...
	PMDG_NG3_Control Control; //Переменная для управления B737NGX
	PMDG_NG3_Data NGX; //Переменная для приема данных B737NGX
	...
	void AddText(String^ text, bool simDisplay=false) {	
		if (MainForm::mainForm->textBox1->InvokeRequired) {
			...
			if (simDisplay) {
				...
				int szz = text->Length;
				//SimConnect_Text(hSimConnect, SIMCONNECT_TEXT_TYPE_PRINT_RED, 10, EVENT_TEXT, sss->Length + 1, ctext);
			}
	...
	bool setNGXDataShort(int Event, short Parameter, short* DefaultParameter, bool direct = FALSE) {
		static int Counter = 0;
		while (*DefaultParameter != Parameter) {
      if (!direct) {
        if (Control.Event == 0) {
