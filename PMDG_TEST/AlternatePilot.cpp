...
namespace PMDG_TEST
{
	...
	double AltPitchWithPos(double TargetValue)
	{
		...
		alterPid lp = PPID;
		/*double pTerm = 0, dTerm = 0, iTerm = 0;
		double error = TargetValue - PaircraftData->pitch;

		pTerm = pidPitch.pGain * error;    // calculate the proportional term

		pidPitch.iState += error;          // calculate the integral state with appropriate limiting
		if (pidPitch.iState > pidPitch.iMax)
			pidPitch.iState = pidPitch.iMax;
		else if (pidPitch.iState < pidPitch.iMin)
			pidPitch.iState = pidPitch.iMin;
		iTerm = pidPitch.iGain * pidPitch.iState;    // calculate the integral term

		dTerm = pidPitch.dGain * (PaircraftData->pitch - pidPitch.dState);
		pidPitch.dState = PaircraftData->pitch;
		double elev = pTerm + iTerm - dTerm;
		/*if (elev >(PaircraftData->elev*16383 + 2000))
		{
			elev = PaircraftData->elev * 16383 + 2000;
		}
		else if (elev <(PaircraftData->elev * 16383 - 2000))
		{
			elev = PaircraftData->elev * 16383 - 2000;
		}*/
		/*if (elev >(16383))
	...
	double AltVSpeedWithPitch(double TargetValue)
	{
		...
		/*double pTerm = 0, dTerm = 0, iTerm = 0;
		double error = TargetValue - PaircraftData->VS*60.0;

		pTerm = pidVSpeed.pGain * error;    // calculate the proportional term

		pidVSpeed.iState += error;          // calculate the integral state with appropriate limiting
		if (pidVSpeed.iState > pidVSpeed.iMax)
			pidVSpeed.iState = pidVSpeed.iMax;
		else if (pidVSpeed.iState < pidVSpeed.iMin)
			pidVSpeed.iState = pidVSpeed.iMin;
		iTerm = pidVSpeed.iGain * pidVSpeed.iState;    // calculate the integral term

		dTerm = pidVSpeed.dGain * (PaircraftData->VS*60.0 - pidVSpeed.dState);
		pidVSpeed.dState = PaircraftData->VS*60.0;
		double elev = pTerm + iTerm - dTerm;
		/*if (elev >(PaircraftData->elev*16383 + 2000))
		{
		elev = PaircraftData->elev * 16383 + 2000;
		...
		//Инвертирование знака
		double intpParameter = -elev;
		AltPitchWithPos(intpParameter);
		pidVSpeed.Term = elev / 20 * 30;
		pidVSpeed.val = PaircraftData->VS*60.0;
